...
const getRom1Bank = () => {
  const romBank = (registers.bankHighBits << 5) | registers.bankLowBits;
  return romBank & (cartridge.getROMSizeAndBanks()[1] - 1);
};

const getRamBank = () => {
  return registers.bankingMode === RAM_AND_ADVANCED_ROM_BANKING_MODE
    ? registers.bankHighBits & (cartridge.getRAMSizeAndBanks()[1] - 1)
    : 0;
};
...
const getRom1Offset = () => {
  return getRom1Bank() * ROM_BANK_SIZE;
};
...
const read = address => {
  if (address >= ROM0_START_ADDR && address <= ROM0_END_ADDR) {
    ...
    return cartridge.read(getRom1Offset() + (address - ROM_BANK_SIZE));
  } else if (
...
const write = (address, value) => {
  if (
    ...
    if ([0x1, 0x2, 0x3].includes(cartridge.getTypeNumber())) {
      registers.bankLowBits = value & 0x1f || 1; // keep 5 lower bits only and set to 1 if 0
    }
  } else if (
    ...
    if ([0x1, 0x2, 0x3].includes(cartridge.getTypeNumber())) {
      registers.bankHighBits = value & 0x03; // keep bits 0 and 1
    }
  } else if (
